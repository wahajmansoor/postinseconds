        ...
        <div className="mx-auto my-auto w-full max-w-md space-y-8 py-8">
          ...
          {isAuthenticated && user ? (
            ...
            <div className="space-y-4">
              {/* PRIMARY GOOGLE SIGN-IN — Google's own real button once
                  Identity Services loads (has to be: it's a secure iframe,
                  can't be triggered by a custom button's click), styled via
                  GIS's own options to sit close to this page's look. Empty
                  and invisible until then, in which case the custom button
                  below (the original design, using the redirect flow) is
                  what actually shows and works — e.g. if an ad blocker
                  kept Google's script from loading at all. Always empty on
                  native, where the custom button below is the only one. */}
